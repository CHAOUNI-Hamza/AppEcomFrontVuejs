...
<form @submit.prevent="submit()">
    <div class="mb-3">
        ...
        v-model="user.email" type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Email">
    </div>
    <div class="mb-3">
        <input 
        @blur="v$.user.password.$touch"
        :class="{ 'text-fields-error' : v$.user.password.$error === true }"
        v-model="user.password" type="password" class="form-control" id="exampleInputPassword1" placeholder="Password">
    </div>
    <div class="mb-3 form-check">
        <a class="link-forgot" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal">Forgot your password?</a>
    </div>
        ...
        <div class="modal-dialog">  
            ...
            <div class="modal-body">
                ...
                <form @submit.prevent="forgoutPassword()">
                    <div class="mb-3">
                        <input @blur="v$.forgotpassword.email.$touch" :class="{ 'text-fields-error' : v$.forgotpassword.email.$error === true }" v-model="forgotpassword.email" type="email" class="form-control" id="exampleFormControlInput1" placeholder="Email">
...
export default {
    ...
    validations () {
        return {
            user: {
                password: { required, minLength: minLength(5), maxLength: maxLength(30), $autoDirty: true },
    ...
    methods: {
       ...
       forgoutPassword() {
        this.v$.$touch();
        if(!this.v$.$invalid) {
                axios.post('auth/forgot-password', this.forgotpassword)
            .then( res => {
